...
var findMin = function(nums) {
    ...
    // 初始化两个指针，left 指向数组的第一个元素（索引为 0），right 指向数组的最后一个元素。
    while(left + 1 < right){
      // 当 left 指针和 right 指针之间至少间隔一个元素时，继续循环。
      let mid = Math.floor((left+right)/2)
      // 计算 left 和 right 指针的中间位置，并向下取整得到 mid 指针。
    ...
    // 循环结束后，left 和 right 指针相邻，返回它们指向元素中的较小值，即为数组中的最小元素。
}; // 函数定义结束