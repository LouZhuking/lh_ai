...
var strStr = function strStr(haystack, needle) {
  ...
  var getNext = function getNext(needle) {
    // 2.1初始化数据
    var next = new Array(needle.length).fill(0); // 创建next数组，长度与needle相同，初始值全为0

    var j = 0; // j指针：指向前缀末尾位置，也表示当前最长公共前后缀长度
  ...
  var next = getNext(needle); // 先计算needle的next数组

  var j = 0; // j指针：指向needle中当前比较的位置
  // 遍历主字符串haystack
