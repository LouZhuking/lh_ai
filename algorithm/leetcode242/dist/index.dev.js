 ...
 * @return {boolean}
 */
var isAnagram = function isAnagram(s, t) {
  ...
  var table = new Array(26).fill(0); // 3. 遍历字符串s 
  ...
  for (var _i = 0; _i < t.length; _i++) {
    table[s.codePointAt(_i) - 'a'.codePointAt(0)]--; // 4.1 table[s.codePointAt(i)-'a'.codePointAt(0)--]-- 从哈希表中减去1
    // 4.2 如果哈希表对应的值小于0 说明t字符串有一个字符不在s字符串中

    if (table[s.codePointAt(_i) - 'a'.codePointAt(0)] < 0) {
      return false;
    }
  }
