...
var GAME_HEIGHT = 600;
var HOOK_SPEED = 5;
var HOOK_SWING_SPEED = 0.03;
var INITIAL_TIME = 60; // 初始时间(秒)

var BASE_SCORE_REQUIREMENT = 500; // 基础过关分数
// 游戏状态
...
function initGame() {
  ...
  gameState.canvas.height = GAME_HEIGHT; // 加载高分

  elements.highScoreDisplay.textContent = gameState.highScore; // 设置事件监听器
  ...
  elements.soundButton.addEventListener('click', toggleSound); // 绘制开始界面
...
function startGame() {
  ...
  gameState.items = []; // 更新显示

  updateScore();
  updateTime();
  elements.levelDisplay.textContent = gameState.level; // 生成物品

  generateItems(); // 隐藏菜单，显示游戏界面

  elements.startMenu.classList.add('hidden');
  elements.endMenu.classList.add('hidden');
  elements.gameScreen.classList.remove('hidden'); // 开始计时器

  if (gameState.timer) clearInterval(gameState.timer);
  gameState.timer = setInterval(updateGameTime, 1000); // 开始游戏循环
...
function generateItems() {
  gameState.items = [];
  var itemCount = 8 + gameState.level * 2;
  var itemTypes = Object.values(ITEM_TYPES); // 确保至少有一个高分值物品

  gameState.items.push(createItem(ITEM_TYPES.GOLD_LARGE));
  gameState.items.push(createItem(ITEM_TYPES.DIAMOND)); // 随机生成其他物品
...
function createItem(itemType) {
  // 确保物品不会重叠太多
  var x, y, overlap;
  var padding = itemType.size * 2;

  do {
    overlap = false;
    x = Math.random() * (GAME_WIDTH - 100) + 50;
    y = Math.random() * (GAME_HEIGHT - 300) + 250; // 检查与已有物品的重叠
    ...
    try {
      for (var _iterator = gameState.items[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
...
function gameLoop() {
  if (!gameState.isPlaying || gameState.isPaused) return; // 清空画布

  gameState.ctx.clearRect(0, 0, GAME_WIDTH, GAME_HEIGHT); // 绘制背景
...
function drawBackground() {
  ...
  gameState.ctx.fillRect(0, 0, GAME_WIDTH, 100); // 绘制矿洞墙壁

  gameState.ctx.fillStyle = '#795548';
  gameState.ctx.fillRect(0, 100, 50, GAME_HEIGHT - 100);
  gameState.ctx.fillRect(GAME_WIDTH - 50, 100, 50, GAME_HEIGHT - 100); // 绘制矿洞地面

  gameState.ctx.fillStyle = '#4e342e';
  gameState.ctx.fillRect(0, GAME_HEIGHT - 50, GAME_WIDTH, 50); // 添加一些岩石纹理

  gameState.ctx.fillStyle = '#6d4c41';

  for (var i = 0; i < 10; i++) {
    var x = Math.random() * (GAME_WIDTH - 100) + 50;
    var y = Math.random() * (GAME_HEIGHT - 200) + 150;
...
function drawMiner() {
  ...
  gameState.ctx.fill(); // 矿工身体

  gameState.ctx.fillStyle = '#d35400';
  gameState.ctx.fillRect(GAME_WIDTH / 2 - 10, 95, 20, 30); // 矿工手臂
...
function updateHook() {
  if (!gameState.hook.isFlying) {
    ...
    gameState.hook.x = GAME_WIDTH / 2 + Math.sin(gameState.hook.angle) * 100; // 到达摆动极限时反向
    ...
    if (!gameState.hook.target) {
      // 向下移动
      gameState.hook.y += gameState.hook.speed; // 检查是否碰到地面或物品
      ...
      // 回收物品
      var dx = GAME_WIDTH / 2 - gameState.hook.x;
      var dy = gameState.hook.y - 100;
      var distance = Math.sqrt(dx * dx + dy * dy); // 根据物品重量调整速度

      var speedFactor = gameState.hook.target.weight || 1;
      var moveSpeed = HOOK_SPEED / speedFactor;
...
function drawHook() {
  ...
  gameState.ctx.rotate(Math.atan2(gameState.hook.y - 100, gameState.hook.x - GAME_WIDTH / 2)); // 绘制抓钩

  gameState.ctx.fillStyle = '#795548';
  gameState.ctx.fillRect(-gameState.hook.width / 2, 0, gameState.hook.width, gameState.hook.height); // 绘制钩子
...
function drawItems() {
  ...
  try {
    for (var _iterator2 = gameState.items[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
      ...
      if (!item.collected) {
        ...
        gameState.ctx.fill(); // 如果是特殊道具，添加标记
...
function checkItemCollision() {
  var hookTipX = gameState.hook.x;
  var hookTipY = gameState.hook.y + gameState.hook.height;
  var _iteratorNormalCompletion3 = true;
  var _didIteratorError3 = false;
  var _iteratorError3 = undefined;

  try {
    for (var _iterator3 = gameState.items[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
      var item = _step3.value;

      if (!item.collected) {
        var dx = hookTipX - item.x;
        var dy = hookTipY - item.y;
        var distance = Math.sqrt(dx * dx + dy * dy);
...
function collectItem(item) {
  if (item.value > 0) {
    ...
    updateScore();
    playSound('collect'); // 如果是金块，增加金币计数
...
function checkLevelUp() {
  var requiredScore = BASE_SCORE_REQUIREMENT * gameState.level;

  if (gameState.score >= requiredScore) {
    gameState.level++;
    elements.levelDisplay.textContent = gameState.level;
    showMessage("\u8FDB\u5165\u7B2C".concat(gameState.level, "\u5173!"), '#ffd700');
...
function endGame(isWin) {
  ...
  clearInterval(gameState.timer); // 更新最高分
  ...
  elements.highScoreDisplay.textContent = gameState.highScore; // 显示结束界面
...
function drawStartScreen() {} // 可以添加一些动画效果
// 播放音效 (简化版)


function playSound(type) {
  if (!gameState.isSoundOn) return; // 在实际项目中这里会播放真实音效

  console.log("\u64AD\u653E\u97F3\u6548: ".concat(type));
} // 当页面加载完成后初始化游戏
