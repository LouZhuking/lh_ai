...
const Coze = () => {
  const VITE_PAT_TOKEN = import.meta.env.VITE_PAT_TOKEN;
  const uploadUrl = 'https://api.coze.cn/v1/files/upload';
  const workflowUrl = 'https://api.coze.cn/v1/workflow/run';
  const workflow_id = '7533135063622959147';
  ...
  const updateImageData = () => {
    ...
    reader.readAsDataURL(file);   // 实时图片显示，以html5的方式介绍
    reader.onload = (e) => {
      // base64格式图片
  ...
  const uploadFile = async () => {
    ...
    const res = await fetch(uploadUrl, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${VITE_PAT_TOKEN}`, },
      body: formData,
    });
  ...
  const generate = async () => {
    ...
    const res = await fetch(workflowUrl, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${VITE_PAT_TOKEN}`,
