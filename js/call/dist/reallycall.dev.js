...
Function.prototype.myCall = function (context) {
  ...
  var fnKey = Symbol("fn"); // 将调用 myCall 方法的函数赋值给 context 的 fnKey 属性

  context[fnKey] = this; // 在 context 上下文中调用函数，并传入参数 ...args，将结果存储在 result 中

  for (var _len = arguments.length, args = new Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
    args[_key - 1] = arguments[_key];
  }

  var result = (_context = context)[fnKey].apply(_context, args); // 删除 context 上临时添加的属性，避免污染 context 对象


  delete context[fnKey]; // 返回函数执行的结果
